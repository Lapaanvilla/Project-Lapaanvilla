...
class Driver_api extends REST_Controller {
    function __construct()
    {
        ...
        parent::__construct();
        $this->load->model('v2/driver_api_model');
            ...
            if ($tokenres->status == 1)
            {
                if($order_id)
                {
                    ...
                    if(!empty($details))
                    {
                        ...
                        if(!empty($userData) && $userData->notification==1)
                        {
                            //Notification code Start
                            $topic_val = sprintf(NOFICAITON_TOPIC,$userData->entity_id);
                            ...
                            $fields_data = array ('screenType'=>'order','order_id'=>trim($order_id),'wallet_amount'=>trim($new_wallet_amount),'sound'=>'default');
