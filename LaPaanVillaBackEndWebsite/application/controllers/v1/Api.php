...
class Api extends REST_Controller {
               ...
               /* $this->db->select('OptionValue');
                ...
                if(!empty($itemDetail)){
                    foreach ($itemDetail['items'] as $key => $value)
                    {
                        ...
                        $getMenuID = $this->api_model->getMenuID($this->current_lang,$menu_content_id,$restaurant_id);
                        if (!empty($getMenuID)) {
                            $allow_scheduled_delivery = $taxdetail->allow_scheduled_delivery;
                            if($order_delivery=='DineIn')
                            {
    ...
    public function addRequestQR_post()
    {
        ...
        {    
            ...
            $chktbl_availability = $this->api_model->getRecordreserveTable($table_id,$qr_code_detail->resto_entity_id,$user_id);
